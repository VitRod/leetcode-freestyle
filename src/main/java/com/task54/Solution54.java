...
public class Solution54 {
	...
	public List<Integer> spiralOrder(int[][] matrix) {
        ...
        if (matrix.length == 0) return res; // if matrix is empty, return empty list
        ...
        for (int i = 0; i < m * n; i++) { // loop until all elements are visited
            ...
            if (nx < 0 || nx >= m || ny < 0 || ny >= n || visited[nx][ny] == 1) { // if next x or y is out of bound or next element is visited
