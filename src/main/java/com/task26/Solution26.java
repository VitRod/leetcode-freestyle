...
// if the array is empty or has only one element, return the length
        if(len<=1) return len;  
        int tail = 1; // tail of the array
        for(int i = 1; i<len; ++i){ // iterate through the array
//if the previous element is not equal to the current element
            if(nums[i-1] != nums[i]){ 
  // add the current element to the tail of the array
            nums[tail++]= nums[i];
            }
        }
        return tail; // return the length of the array
    }
}
