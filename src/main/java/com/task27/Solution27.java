...
public class Solution27 {
	...
	public int removeElement(int[] nums, int val) {
		int tail = 0; // tail of the array
		for (int i = 0, len = nums.length; i < len; i++) { // iterate through the array
			if (nums[i] != val) { // if the element is not equal to the value
				nums[tail++] = nums[i]; // add the element to the tail of the array
			}
		}
		return tail; // return the tail of the array
	}
}
